...
import { Clock, ArrowLeft, Award, CheckCircle2, BookOpen, ChevronRight, ArrowRight, Crown } from "lucide-react";
...
export function LessonSidebar({ currentLesson, lessonsCompleted }: LessonSidebarProps) {
  ...
  const isCompleted = lessonsCompleted.includes(currentLesson.id);
  const trackColor = getTrackHsl(currentLesson.track);
  const trackName = getTrackName(currentLesson.track);
  useEffect(() => {
    async function loadTrackItems() {
      if (!moduleId) return;
      const index = await loadContentIndex();
      const lessons = index.lessons.filter((l) => l.module === moduleId && l.track === currentLesson.track);
      const challenges = index.challenges.filter((c) => c.module === moduleId && c.track === currentLesson.track);
    ...
    <div className="space-y-6 w-full min-w-0">
      ...
      <Card className={`shadow-card border-accent/20 ${
        ...
        <CardHeader className="pb-2 text-center">
          ...
          <CardTitle className="text-lg break-words">{currentLesson.title}</CardTitle>
        </CardHeader>
      ...
      {/* Back to Explore */}
      <Link to={`/module/${moduleId}/explore`} className="block">
                ...
                <div className="space-y-1.5">
                  {trackItems.required.map((item) => (
                    <TrackItemRow
                      ...
                      isCurrent={item.type === "lesson" && item.id === currentLesson.id}
                      moduleId={moduleId!}
